...
const Appointment = () => {
  ...
  const bookAppointment = async () => {
    ...
    try {
      ...
      const {data} = await axios.post(backendUrl + '/api/user/bool-appointment', {docId, slotDate, slotTime}, {headers: {token}});
      if(data.success){
      ...
      <div className="flex flex-col sm:flex-row gap-4">
        ...
        <div className="flex-1 border border-gray-400 rounded-lg p-8 py-7 bg-white mx-2 sm:mx-0 mt-[-80px] sm:mt-0">
          ...
          <div className="flex flex-col gap-1 text-sm font-medium text-gray-900 mt-3">
            <p className="flex gap-2">About <img src={assets.info_icon} alt="info" /></p>
        ...
        <div className="flex items-center gap-3 w-full overflow-x-scroll mt-4">
            {
                docSlots.length && docSlots[slotIndex].map((item, idx) => (
                    <p onClick={() => setSlotTime(item.time)} className={`text-sm font-light flex-shrink-0 px-5 py-2 rounded-full cursor-pointer ${item.time === slotTime ? 'bg-[#5f6FFF] text-white' : 'text-gray-400 border border-gray-300' }`} key={idx}>
                        {
