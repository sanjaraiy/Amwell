        ...
        <div>
            ...
            <div className="flex flex-col sm:flex-row items-start gap-5 mt-5">
              <button className={`py-1 px-3 border rounded text-sm transition-all sm:hidden ${showFilter ? 'bg-[#5f6FFF] text-white' : ''}`} onClick={() => setShowFilter((prev) => !prev)}>Filters</button>
                ...
                <div className="w-full grid grid-cols-[repeat(auto-fill,_minmax(200px,_1fr))] gap-4 gap-y-4">
                    {
                    ...
                    <div onClick={()=>navigate(`/appointment/${item._id}`)} className='border border-blue-200 rounded-xl overflow-hidden cursor-pointer hover:translate-y-[-10px] transition-all duration-500' key={idx}>
                        ...
                        <div className='p-4'>
                            <div className={`flex items-center gap-2 ${item.available ? 'text-green-500' : 'text-gray-500'} text-sm text-center text-green-500`}>
